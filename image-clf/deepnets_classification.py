...
filepath=job_root+'/checkpoints/model_'+sys.argv[1][-15:-5]+".hdf5"
...
class accHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self,epoch,logs):
        ...
        f = open(job_root+'/logs/acc_log-'+sys.argv[1][-15:-5]+'.json',"w")
...
class stopExplodingLoss(keras.callbacks.Callback):
    ...
    def on_epoch_end(self,epoch,logs={}):
        ...
        if val_loss>self.max_loss:
            ...
            #sys.exit("Training interrupted, loss if too high")

stoploss = stopExplodingLoss(args['num_classes'])

history = accHistory(model_id,args)

stopnan = keras.callbacks.TerminateOnNaN()

earlystop = EarlyStopping(monitor='val_loss',min_delta = 1e-3,patience=3,mode='min',restore_best_weights=True)
...
model.fit_generator(train_generator,
                    ...
                    callbacks=[tensorboard,checkpoint,history,earlystop,stoploss,stopnan])

score = model.evaluate_generator(test_generator,steps=2160//batch_size,max_queue_size=6)

print("Saving grapg and weights:")
model_json = model.to_json()

with open(job_root+'/graphs/model_'+sys.argv[1][-15:-5]+'.json',"w") as json_file:
    json_file.write(model_json)

model.save_weights(job_root+'/weights/model_'+sys.argv[1][-15:-5]+'.h5')
